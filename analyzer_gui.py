...
for ind in range(0, wine_csv.shape[0]):
    totscore = 0
    totscore = wine_csv.loc[ind]['score']*3.5
    if wine_csv.loc[ind]['price'] is not None:
        if wine_csv.loc[ind]['price'] > 250:
            ...
            totscore = totscore + 6 * 3
        elif wine_csv.loc[ind]['price'] > 60:
            totscore = totscore + 7 * 3
        elif wine_csv.loc[ind]['price'] > 40:
            totscore = totscore + 8 * 3
        elif wine_csv.loc[ind]['price'] > 20:
            totscore = totscore + 9 * 3
        elif wine_csv.loc[ind]['price'] > 0:
            totscore = totscore + 10 * 3
    ...
    if wine_csv.loc[ind]['variety'] is not None:
        if pd.isnull(wine_csv.loc[ind]['variety'])==False:
            totscore = totscore + variety_csv.loc[wine_csv.loc[ind]['variety']]*2
    totscore = round(totscore/10,2)
...
fig = go.Figure(data=[go.Table(
    header=dict(values=list(wine_csv.columns),
                fill_color='paleturquoise',
                align='left'),
    cells=dict(values=[wine_csv.id, wine_csv.country, wine_csv.title, wine_csv.score, wine_csv.price, wine_csv.state, wine_csv.region_1, wine_csv.region_2, wine_csv.variety, wine_csv.winery, wine_csv.totalscore],
               fill_color='lavender',
               align='left'))
])
fig.show()
...
fig1 = go.Figure(data=[go.Bar(
            x=wine_csv4view.title, y=wine_csv4view.totalscore,
            text=wine_csv4view.totalscore,
            textposition='auto',
        )], layout=layout)
fig1.show()